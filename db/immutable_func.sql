...
-- but IMMUTABLE.
CREATE OR REPLACE FUNCTION {{ schema }}.parse_iso_date(p_text text)
RETURNS date
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION {{ schema }}.trunc_year(p_date date)
RETURNS date
LANGUAGE sql
IMMUTABLE
AS $$
...
-- DateStyle settings.
CREATE OR REPLACE FUNCTION {{ schema }}.date_to_iso(p_date date)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
